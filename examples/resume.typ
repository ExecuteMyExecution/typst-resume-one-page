...
#show: resume-header.with(
  author: "姚宗宇",
  profile-image: "/assets/avatar.jpg",
  basic-info: ([#text(font: "Noto Serif CJK SC", weight: "bold")[男 · 25岁 · 本科]],),
...
#resume-entry(
  ...
  - #emph[GPA: 3.61/4.33] #h(0.5em) 专业前10%；人民奖学金；优秀毕业生
]
...
- *编程语言*：Go、Python、Java、PHP（具备阅读与开发能力，云基础设施方向以Python/Shell为主）
...
#resume-entry(
  ...
  - 负责海外自动驾驶地图产线基础设施建设，覆盖中东、欧洲、韩国3个区域及AWS、阿里云多云环境，服务稳定性99%+，0重大故障。
...
#resume-entry(
  ...
  - 负责存储改造（AFS/Table → S3）及国际化部署，推进多模块在AWS环境的部署联调，打通OTA发布流程。
...
#resume-entry(
  ...
  - 独立承接智驾数据可视化平台（DV平台）需求开发与重构，缩短交付周期并提升平台用户满意度。
]
...
#resume-entry(
  ...
  - #emph[背景]：海外产线横跨AWS和阿里云，基础设施手工管理效率低、配置漂移难追踪。
  ...
  - #emph[结果]：实现AWS 3个区域 + 阿里云1个区域的基础设施全量IaC化，配置变更全程可审计、可回滚。
]
...
#resume-entry(
  ...
  - #emph[背景]：产线前期为快速上线未做精细化资源管理，ELB、数据库、计算实例等成本偏高。
  - 收敛Ingress入口（41个→8个），减少ELB资源占用，成本降低80%+。
  - 设计AWS Lambda + Schedule方案，实现RDS/DocumentDB/Redis非工作时段定时启停。
  - 落地cronhpa-controller + Karpenter弹性扩缩容，降低计算节点闲时成本。
  - 将EFS目录打包至基础镜像并配置S3存储过期策略，压缩存储成本。
  - #emph[结果]：综合固定成本降低1000+\$/天，数据库及常驻服务成本各节省约1/3。
]
...
#resume-entry(
  ...
  - #emph[背景]：支持自动驾驶地图业务海外扩张，需在AWS中东/欧洲/韩国及阿里云环境复刻国内地图生产产线。
  - 设计存储改造方案（AFS/Table→S3），完成rowKey→s3Key转换及MySQL元信息接入。
  - 负责后处理、外参服务、解算平台等多模块部署联调，完成镜像依赖替换及HTTPS零信任域名适配。
  - 完成AWS向阿里云的迁移，打平不同运行环境镜像，实现一套代码库无缝支撑混合云部署。
  - #emph[结果]：所有区域全流程打通，实现服务快速部署以及产线合规交付高精地图，链路稳定性99%+。
]
